...
#                      input_window_samples=timepoints, final_fc_length='auto')
...
model = ConTransformer(drop_prob=0.5, seq_len=timepoints,
...
hyperparameter_combinations = list(itertools.product(
    learning_rates, embedding_dims, dim_ffs, weight_decays, dropouts))

# Loop through each hyperparameter combination
for lr, emb_dim, dim_ff, wd, dropout in hyperparameter_combinations:
    # Create model with current hyperparameters
    model = NewTransformer(embedding_dim=emb_dim, vocab_size=vocab_size, dim_ff=dim_ff,
                           seq_len=channels * timepoints, dropout=dropout)
    ...
    print(
        f"lr:{lr} emd_dim:{emb_dim} dim_ff:{dim_ff} weight decay:{wd} dropout:{dropout}")
    trainer.fit(epochs=16)
# %%
x, y = next(iter(train_dataloader))
